...
class SACPolicy(TFPolicy):
    ...
    def calculate_loss(self, experiences_in):
        ...
        if self.use_vec_obs:
            ...
            feed_dict[self.model.next_vector_in] = experiences[
                "next_vector_in"
        ...
        feed_dict[self.model.is_weights] = np.ones(
            len(experiences_in), dtype=np.float64
        )
        out_dict = {"q1_loss": self.model.q1_loss_per_element}
